...
PathFinding::PathFinding(App* application, bool start_enabled) : Module(application, start_enabled), map(NULL), lastPath(DEFAULT_PATH_LENGTH), width(0), height(0)
{
...
int PathFinding::CreatePath(const iPoint& origin, const iPoint& destination)
{
	...
	while (open.list.Count() > 0) {
		...
		if (node->data.pos==destination) {
			...
			const PathNode* pahtNode = &node->data;

			while (pahtNode != nullptr){
				lastPath.PushBack(pahtNode->pos);
				pahtNode = pahtNode->parent;
			}
